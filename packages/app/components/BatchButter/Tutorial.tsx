...
function ProgressDot(filled: boolean): JSX.Element {
  return (
    <div className="bg-primarlyLight w-3 h-3 rounded-full border border-gray-800 flex justify-center items-center">
      {filled && <div className="w-2 h-2 rounded-full bg-gray-800"></div>}
    </div>
  );
}
  ...
  return (
    <div className="w-full h-full flex flex-row">
      <div className="w-2/12 flex items-center justify-center">
        <button
          className="w-24 h-24 rounded-full bg-white opacity-50 flex justify-center items-center shadow-custom hover:opacity-70"
      ...
      <div className="w-8/12">
        ...
        <h2 className="font-semibold leading-none text-center">How it works</h2>
        <h1 className="font-bold leading-none text-center text-2xl mt-8">
          {title(step)}
        </h1>
        <p className="text-center mt-4">{text(step)}</p>
        <div className="flex flex-row items-center justify-evenly mx-auto w-2/12 mt-7">
          {ProgressDot(true)}
          {ProgressDot(step > 1)}
          {ProgressDot(step === 3)}
        </div>
