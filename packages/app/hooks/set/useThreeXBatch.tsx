...
export default function useThreeXBatch(): ThreeXBatchProcessing {
  const { signerOrProvider, contractAddresses, account, chainId } = useWeb3();

  return useMemo(() => {
    if (isButterSupportedOnCurrentNetwork(chainId))
      return ThreeXBatchProcessing__factory.connect(contractAddresses.threeXBatch, signerOrProvider);
