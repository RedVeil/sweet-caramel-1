...
async function getArrakisUnderlyingAmounts(
  ...
  const arrakisVault = new Contract(address, ["function getUnderlyingBalances() view returns (uint256, uint256)"], provider);
...
async function getLPTokenPrice(
  ...
  const [usdcAmount, popAmount] = address.toLowerCase() === popUsdcArrakisVault.toLowerCase() ?
...
async function getSetTokenPrice(
  ...
  const [addresses, quantities] = await basicIssuanceModule.getRequiredComponentUnitsForIssue(
    address,
    parseEther("1"),
  );
...
export default function useTokenPrices(addresses: string[], chainId: ChainId) {
  ...
  const shouldFetch = !!provider && lpToken.length > 0 && (chainId === ChainId.Ethereum ? (vaults.length > 0 && setToken.length > 0 && basicIssuanceModule) : true)
  ...
  async function fetchPrices(tokenAdresses: string[], chain: ChainId): Promise<{ [x: string]: BigNumber }> {
    const internalToken = tokenAdresses
      .filter((address) => isInternalToken(address)).length > 0 ? await tokenAdresses
    ...
    const externalTokens = tokenAdresses.filter((address) => !isInternalToken(address)).length > 0 ? await getAssetValue(
  ...
  return useSWR(shouldFetch ? ["assetValue", addresses, chainId] : null, (key, addresses, chainId) => fetchPrices(addresses, chainId), {
