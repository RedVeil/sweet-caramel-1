...
export default function useNetWorth(): BigNumber {
  ...
  const butterHoldings = useMemo(
    () => (butterBalance && butterPrice ? getHoldingValue(butterBalance, butterPrice) : constants.Zero),
    [butterBalance, butterPrice],
  );
  const threeXHoldings = useMemo(
    () => (threeXBalance && threeXPrice ? getHoldingValue(threeXBalance, threeXPrice) : constants.Zero),
    [threeXBalance, threeXPrice],
  );
  const butterStakingHoldings = useMemo(
    () =>
      butterStakingPool && butterPrice ? getHoldingValue(butterStakingPool.userStake, butterPrice) : constants.Zero,
    [butterStakingPool, butterPrice],
  );
  const threeXStakingHoldings = useMemo(
    () =>
      threeXStakingPool && threeXPrice ? getHoldingValue(threeXStakingPool.userStake, threeXPrice) : constants.Zero,
    [threeXStakingPool, threeXPrice],
  );
