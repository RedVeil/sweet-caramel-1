...
function NetworkListComponent({ title, selectedNetworks }: { selectedNetworks: any; title: string }) {
  return (
...
function PortfolioSection({
  ...
  return (
    <section className={`px-4 md:px-8 mt-7 ${showSection || "hidden"}`}>
      <div className={`mt-8 md:hidden`}>
        <NetworkListComponent title={title} selectedNetworks={selectedNetworks} />
      </div>
      <table className={`table w-full table-fixed border-separate border-spacing-y-4`}>
        <thead>
          <tr className="whitespace-nowrap">
            <th className="w-[8rem] md:w-[36rem] opacity-0 md:opacity-100">
              <NetworkListComponent title={title} selectedNetworks={selectedNetworks} />
            </th>
            ...
            <th className="w-[4rem] md:w-auto text-primary text-lg font-medium px-2">
              <div className="flex items-center gap-2">
                <p className="text-primaryLight text-sm md:text-base">Holding %</p>
        ...
        <tbody className={balanceGTZero ? "" : "hidden"}>{children}</tbody>
      </table>
