...
const eth_call =
  (Component: Pop.FC<BigNumberWithFormatted>) =>
    ({ ...props }: Pop.BaseContractProps & { withPrice?: (price: { price?: BigNumber; address?: string; chainId?: Number }) => React.ReactElement }) => {
      const { data, status } = usePrice({ ...props });

      if (props.withPrice) {
        return (
          <>
            {props.withPrice({ price: data?.value, address: props.address, chainId: props.chainId })}
          </>
        );
      }
