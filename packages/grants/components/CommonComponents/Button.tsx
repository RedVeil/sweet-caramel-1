...
const Button: React.FC<ButtonProps> = ({ variant, children, className, onClick, disabled }) => {
  ...
  return (
    <button
      className={`rounded-4xl text-base flex flex-row items-center justify-center font-medium px-8 py-[10px] border transition-all ease-in-out duration-500 ${className} ${disabled ? "cursor-not-allowed" : "cursor-pointer"
        } ${buttonVariant()}`}
      onClick={onClick}
      disabled={disabled}
    >
