...
const YearSpoiler: React.FC<YearSpoilerProps> = ({
  ...
  return (
    <div>
      <a className="text-base cursor-pointer" onClick={() => setGrantVisibility((prevState) => !prevState)}>
        {year}
      </a>
      {showGrants && (
        <ul>
          {grantRounds
            ?.filter((grantRound) => grantRoundFilter.active && grantRound.active)
            .map((grantRound) => (
              <GrantRoundLink
          ...
          {grantRounds
            ?.filter((grantRound) => grantRoundFilter.closed && !grantRound.active)
            .map((grantRound) => (
              <GrantRoundLink
                key={`${grantRound.id}-GrantLink`}
                grantRound={grantRound}
