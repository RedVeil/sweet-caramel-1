...
const GrantRound: React.FC<GrantRoundProps> = ({
  ...
  const [beneficiariesWithMetadata, setBeneficiaries] = useState<BeneficiaryApplication[]>([]);
  ...
  const getAllBeneficiaries = async (registeredBeneficiaries: string[]) => {
    setBeneficiaries(await Promise.all(registeredBeneficiaries.map((address) => getBeneficiary(address))));
  };
