...
async function uploadMultipleFiles(files: File[], setLocalState: (input: string[]) => void, fileType: string) {
  loading();
  ...
  if (uploadResults.every(isSuccessfulUpload)) {
    ...
    toast.dismiss();
    success(`${fileType === "image/*" ? "Images" : "Files"} successfully uploaded to IPFS`);
...
async function uploadMultiplePDFs(files: File[], setLocalState: (input: string[]) => void, fileType: string) {
  loading();
  const uploadResults = await Promise.all(
    files.map((file) => {
      return IpfsClient.upload(file);
...
const IpfsUpload: React.FC<IpfsProps> = ({
  ...
  return (
    <div className="mx-auto">
      <h2 className="text-center text-base text-indigo-600 font-semibold tracking-wide">{stepName}</h2>
              ...
              <div className="max-w-lg flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                <div className="space-y-1 text-center">
                  ...
                  <div className="flex text-sm text-gray-600">
                    ...
                    <p className="pl-1">or drag and drop {fileDescription.toLowerCase()}</p>
                  </div>
      ...
      {videoUploading(uploadProgress, fileType) && (
        <div className="grid my-2 justify-items-stretch">
          <span className="mx-4  w-1/2 justify-self-center flex flex-row justify-between  pb-2">
            <ProgressBar progress={uploadProgress} progressColor={"bg-green-300"} />
          </span>
        </div>
      )}
      {localState && fileType === "video/*" ? <DisplayVideo localState={localState} /> : <> </>}
    </div>
  );
};
