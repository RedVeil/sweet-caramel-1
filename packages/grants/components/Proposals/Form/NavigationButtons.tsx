...
const NavigationButtons: React.FC<NavigationButtonProps> = ({ navigation }) => {
  ...
  const progressPercentage = currentStep === 0 ? 0 : Math.round((100 * currentStep - 1) / numSteps);
    ...
    <footer className="h-10 mb-4 mr-4">
      <div className="grid justify-items-stretch ...">
        <span className="relative z-0 inline-flex shadow-sm rounded-md justify-self-end">
          <div className="mr-2">
            <p className="text-gray-500 text-sm relative inline-flex items-center ">{progressPercentage}% completed</p>
            <ProgressBar progress={progressPercentage} progressColor={"bg-indigo-300"} />
          </div>
          <button
            type="button"
