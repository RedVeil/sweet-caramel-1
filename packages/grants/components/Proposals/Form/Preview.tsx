...
const Preview: React.FC<FormStepProps> = ({ form, navigation, visible }) => {
  ...
  const getListOfIncompleteFormInputs = (submissionData: BeneficiaryApplication): string[] => {
    ...
    if (submissionData.files.headerImage.description === "") incompleteFormInputs.push("Header Image Description");
  ...
  async function uploadJsonToIpfs(submissionData: BeneficiaryApplication): Promise<void> {
    if (await checkPreConditions()) {
      ...
      toast.dismiss();
      await (
        await contracts.pop.connect(library.getSigner()).approve(contracts.beneficiaryGovernance.address, proposalBond)
      ).wait();
  ...
  return (
    visible && (
      <div>
        ...
        {getListOfIncompleteFormInputs(formData).length > 0 && (
          <DisplayIncompleteFormInputs incompleteFields={getListOfIncompleteFormInputs(formData)} />
        )}
