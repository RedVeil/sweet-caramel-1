...
export default function AllGrants() {
  ...
  async function IsUserAlreadyRegistered() {
    const connected = contracts.grantElections.connect(library.getSigner());
    const elections = [
  ...
  useEffect(() => {
    ...
    if (contracts?.grantElections && account) {
  ...
  useEffect(() => {
    ...
    if (contracts?.beneficiaryRegistry && account) {
      let connected = contracts.beneficiaryRegistry.connect(library.getSigner());
      connected
  ...
  useEffect(() => {
    if (contracts?.pop && account) {
      contracts.pop.balanceOf(account).then((res) => console.log("POP Balance: ", res));
    }
  }, [contracts, account]);
  ...
  function assignVotes(grantTerm: number, vote: Vote): void {
    ...
    if (vote.address == pendingVotes[grantTerm].lastAddress) {
      ...
      pendingVotes[grantTerm].lastAmount = vote.votes;
      pendingVotes[grantTerm].total = Object.keys(pendingVotes[grantTerm].votes).reduce((total, address) => {
    ...
    if (pendingVotes[grantTerm].total > voiceCredits) {
      pendingVotes[grantTerm].votes[vote.address] = vote.votes - (pendingVotes[grantTerm].total - voiceCredits);
      pendingVotes[grantTerm].lastAmount = pendingVotes[grantTerm].votes[vote.address];
  ...
  const submitVotes = async (grantTerm: ElectionTerm) => {
    ...
    const txArgs = Object.keys(pendingVotes[grantTerm].votes).reduce<[string[], BigNumber[], number]>(
      (txArgs, address) => {
    ...
    <div className="w-full bg-gray-900 pb-16">
      ...
      <div className="w-10/12 mx-auto mt-8">
        {[...elections]
          ...
          .filter((election) => selectedGrantTerms.includes(election.electionTerm))
          .sort((election1, election2) => Number(election2.startTime) + Number(election1.startTime))
          .map((election) => (
            <ElectionSection
