...
export default function Register(): JSX.Element {
  ...
  useEffect(() => {
    if (contracts && account) {
      contracts.threeCrv.balanceOf(contracts.rewardsManager.address).then((res) => setFeeBalance(res));
      contracts.pop.balanceOf(contracts.rewardsManager.address).then((res) => setPopBalance(res));
      contracts.rewardsManager.getRewardSplits().then((res) => setRewardSplits(res));
    }
  }, [contracts, account]);
  ...
  useEffect(() => {
    if (contracts && feeBalance !== undefined) {
      contracts.uniswapRouter
        .getAmountsOut(feeBalance, [contracts.threeCrv.address, contracts.pop.address])
  ...
  async function swapToken(): Promise<void> {
    ...
    await contracts.rewardsManager
      ...
      .swapTokenForRewards([contracts.threeCrv.address, contracts.pop.address], swapOutput)
      ...
      <div className="bg-gray-900">
        <div className="pt-12 px-4 sm:px-6 lg:px-8 lg:pt-20">
          <div className="text-center">
            ...
            <p className="mt-2 text-3xl font-extrabold text-white sm:text-4xl lg:text-5xl">Rewards Manager</p>
            <p className="mt-3 max-w-4xl mx-auto text-xl text-gray-300 sm:mt-5 sm:text-2xl">
              You can swap 3CRV for POP here to distribute it to the beneficiaries.
            </p>
          </div>
        </div>
        ...
        <div className="mt-8 pb-12 lg:mt-8 lg:pb-20">
          <div className="max-w-2xl mx-auto rounded-lg shadow-xl bg-white">
            <div className="rounded-t-lg pt-12 pb-10">
              <h3 className="text-center text-3xl font-semibold text-gray-900 sm:-mx-6">Swap Accrued Fees</h3>
              <p className="text-lg text-center text-gray-800 mt-4">
                Swap accrued fees to distribute them as rewards across the protocol.
              </p>
            </div>
              ...
              <div className="flex flex-row items-center">
                <div className="flex flex-row items-center">
                  <div className="mr-4 p-2 rounded-md flex flex-row bg-gray-200 border border-gray-400 shadow-sm cursor-pointer">
                    ...
                    <img src="/images/3crv_icon.png" alt="Logo" className="w-6 h-6 mr-2"></img>
        ...
        <div className="mt-8 pb-12 lg:mt-8 lg:pb-20">
          <div className="max-w-2xl mx-auto rounded-lg shadow-xl bg-white">
            <div className="px-4 sm:px-6 lg:px-12 py-8">
              <h3 className="text-center text-3xl font-semibold text-gray-900 sm:-mx-6">Distribute Rewards</h3>
            </div>
            <div className="bg-gray-50 border-t-2 border-gray-100 rounded-b-lg pb-10 px-4 sm:px-6 lg:px-12 pt-12">
              <div className="flex flex-row">
                ...
                <div className="w-5/12 flex flex-col space-y-8 items-end">
                  <RewardDestination destination="Beneficiaries" reward={calculateRewardAmount(3)} />
                  <RewardDestination destination="Staking" reward={calculateRewardAmount(0)} />
                  <RewardDestination destination="Treasury" reward={calculateRewardAmount(1)} />
                  <RewardDestination destination="Insurance" reward={calculateRewardAmount(2)} />
                </div>
              </div>
              <div className="rounded-lg shadow-md mt-8">
                {!account && (
