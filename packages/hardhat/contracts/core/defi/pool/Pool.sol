...
import "openzeppelin-v3/utils/Pausable.sol";
...
contract Pool is AffiliateToken, ReentrancyGuard, Pausable {
  ...
  constructor(
    ...
    AffiliateToken(
      _token,
      _yearnRegistry,
      string(abi.encodePacked("Popcorn ", IERC20Metadata(_token).name(), " Pool")),
   ...
   */
  function deposit(uint256 _amount) public override nonReentrant whenNotPaused blockLocked returns (uint256) {
   ...
   */
  function withdraw(uint256 _amount) public override nonReentrant blockLocked returns (uint256) {
    require(_amount <= balanceOf(msg.sender), "Insufficient pool token balance");
    ...
    address rewardsManager = contractRegistry.getContract(keccak256("RewardsManager"));
   ...
   */
  function transfer(address _recipient, uint256 _amount) public override blockLocked returns (bool) {
  ...
  function withdrawAccruedFees() external {
    ...
    _withdraw(address(this), contractRegistry.getContract(keccak256("RewardsManager")), valueFor(balance), true);
  }
  ...
  function _depositFor(uint256 _amount, address _recipient) internal returns (uint256) {
  ...
  function _issuePoolTokensForAmount(address _to, uint256 _amount) internal returns (uint256) {
  ...
  function _takeManagementFee() internal {
    ...
    uint256 fee = (managementFee.mul(totalValue()).mul(period)).div(MINUTES_PER_YEAR.mul(BPS_DENOMINATOR));
    if (fee > 0) {
  ...
  function _takePerformanceFee() internal {
    if (pricePerPoolToken() > poolTokenHWM) {
      ...
      uint256 fee = performanceFee.mul(changeInPricePerToken).mul(totalSupply()).div(BPS_DENOMINATOR).div(1e18);
      _issuePoolTokensForAmount(address(this), fee);
  ...
  function _issuePoolTokens(address _to, uint256 _amount) internal returns (uint256) {
    _mint(_to, _amount);
    return _amount;
  }
