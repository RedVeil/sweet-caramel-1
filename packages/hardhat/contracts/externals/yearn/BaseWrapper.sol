...
abstract contract BaseWrapper is Initializable {
  ...
  function totalVaultBalance(address account) public view returns (uint256 balance) {
    ...
    for (uint256 id = 0; id < vaults.length; id++) {
      balance =
        balance +
        ((vaults[id].balanceOf(account) * vaults[id].pricePerShare()) / (10**uint256(vaults[id].decimals())));
    }
  }
    ...
    for (uint256 id = 0; id < vaults.length; id++) {
      ...
      if (availableShares > 0) {
        ...
        if (amount != WITHDRAW_EVERYTHING) {
          ...
          uint256 estimatedShares = ((amount - withdrawn) * (10**uint256(vaults[id].decimals()))) /
            vaults[id].pricePerShare(); // NOTE: Changes every iteration // NOTE: Every Vault is different
