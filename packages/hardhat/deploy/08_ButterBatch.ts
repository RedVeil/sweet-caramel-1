...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (["hardhat", "local"].includes(hre.network.name)) {
    await keeperIncentive.updateIncentive(butterBatchProcessing.address, 0, 0, true, true, pop, 1, 0);
    await keeperIncentive.updateIncentive(butterBatchProcessing.address, 1, 0, true, true, pop, 1, 0);
