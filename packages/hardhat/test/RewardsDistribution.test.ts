...
describe("RewardsDistribution", function () {
  beforeEach(async () => {
    ...
    staking = await (
      ...
      ).deploy(mockPop.address, mockPop.address, rewardsEscrow.address)
    ).deployed();
  ...
  describe("approveRewardDistributor", async () => {
    it("should allow owner to add an address as an approved reward distributor", async () => {
      await rewardsDistribution
        .connect(owner)
        .approveRewardDistributor(rewardsDistributor.address, true);
      await expectValue(
        await rewardsDistribution.rewardDistributors(
  ...
  describe("addRewardDistribution", () => {
    context("reverts", () => {
      ...
      it("reverts when amount = 0", async () => {
        expectRevert(
          rewardsDistribution.addRewardDistribution(staking.address, 0, false),
          "Cant add a zero amount"
        );
      });
    });
    context("success", async () => {
      ...
      beforeEach(async () => {
        ...
        result = await rewardsDistribution.addRewardDistribution(
          destAddr,
          amount,
          isLocker
        );
      ...
      it("emits an event", async () => {
        expect(result)
          .to.emit(rewardsDistribution, "RewardDistributionAdded")
          .withArgs(0, destAddr, amount, isLocker);
      });
  ...
  describe("removeRewardDistribution", () => {
    ...
    context("reverts", async () => {
      it("reverts when index out of bounds", async () => {
        expectRevert(
          rewardsDistribution.removeRewardDistribution(2),
          "index out of bounds"
        );
      });
    });
  ...
  describe("editRewardDistribution", () => {
    ...
    context("reverts", () => {
      it("reverts when index out of bounds", async () => {
        expectRevert(
          rewardsDistribution.editRewardDistribution(
            1,
            staking.address,
            parseEther("1"),
            false
    ...
    context("success", async () => {
      ...
      beforeEach(async () => {
        ...
        result = await rewardsDistribution.editRewardDistribution(
          0,
          destAddr,
          amount,
          isLocker
  ...
  describe("distributeRewards", () => {
    context("reverts", () => {
      it("reverts when not called by a rewardsDistributor", async () => {
        expectRevert(
          rewardsDistribution.distributeRewards(parseEther("1")),
          "not authorized"
        );
      });
      it("reverts when amount = 0", async () => {
        expectRevert(
          rewardsDistribution.connect(rewardsDistributor).distributeRewards(0),
          "Nothing to distribute"
        );
      });
      it("reverts when pop is not set", async () => {
        await rewardsDistribution.approveRewardDistributor(
          rewardsDistributor.address,
          true
        );
        await rewardsDistribution.setPop(
          "0x0000000000000000000000000000000000000000"
        );
        expectRevert(
          rewardsDistribution
            .connect(rewardsDistributor)
            .distributeRewards(parseEther("1")),
          "Pop is not set"
        );
      });
      it("reverts when treasury is not set", async () => {
        ...
        await rewardsDistribution.setTreasury(
          "0x0000000000000000000000000000000000000000"
        );
        expectRevert(
          rewardsDistribution
    ...
    context("success", async () => {
      ...
      beforeEach(async () => {
        ...
        await mockPop
          .connect(owner)
          .transfer(rewardsDistribution.address, parseEther("21"));
        result = await rewardsDistribution
          .connect(rewardsDistributor)
          .distributeRewards(parseEther("21"));
      });
      it("emits an event", async () => {
        expect(result)
          .to.emit(rewardsDistribution, "RewardsDistributed")
          .withArgs(parseEther("21"));
      });
      it("calls notifyRewardAmount on each contract", async () => {
        ...
        expect(result)
          .to.emit(popLocker, "RewardAdded")
          .withArgs(mockPop.address, parseEther("10"));
        expect(await mockPop.balanceOf(staking.address)).to.eq(
          parseEther("10")
        );
        expect(await mockPop.balanceOf(popLocker.address)).to.eq(
          parseEther("10")
        );
      });
      it("sends the remainder to the treasury", async () => {
        expect(await mockPop.balanceOf(treasury.address)).to.eq(
          parseEther("1")
        );
      });
      it("skips deleted distributions", async () => {
        ...
        await mockPop
          .connect(owner)
          .transfer(rewardsDistribution.address, parseEther("11"));

        result = await rewardsDistribution
          .connect(rewardsDistributor)
          .distributeRewards(parseEther("10"));
        expect(result)
          .to.emit(popLocker, "RewardAdded")
          .withArgs(mockPop.address, parseEther("10"));
        expect(await mockPop.balanceOf(staking.address)).to.eq(
  ...
  describe("only owner", () => {
    ...
    it("approveRewardDistributor() reverts when not called by owner", async () => {
      expectRevert(
        rewardsDistribution
          .connect(nonOwner)
          .approveRewardDistributor(owner.address, true),
        "Only the contract owner may perform this action"
      );
    });
    it("addRewardDistribution() reverts when not called by owner", async () => {
      expectRevert(
        rewardsDistribution
          .connect(nonOwner)
          .addRewardDistribution(staking.address, parseEther("1"), false),
        "Only the contract owner may perform this action"
      );
    });
    ...
    it("editRewardDistribution() reverts when not called by owner", async () => {
      expectRevert(
        rewardsDistribution
          .connect(nonOwner)
          .editRewardDistribution(0, staking.address, parseEther("1"), false),
        "Only the contract owner may perform this action"
      );
    });
  });
