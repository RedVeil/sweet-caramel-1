...
describe("Vault", function () {
  ...
  describe("constructor", async function () {
    ...
    it("approves staking when staking address is nonzero", async function () {
      ...
      await expectValue(await vault.allowance(vault.address, contracts.staking.address), ethers.constants.MaxUint256);
    });
    ...
    it("approves vault for staking", async () => {
      await expectValue(
        await contracts.vault.allowance(contracts.vault.address, contracts.staking.address),
        MAX_UINT_256
      );
    });
    ...
    it("sets feesUpdatedAt to deployment block timestamp", async function () {
      ...
      await expectValue(await contracts.vault.feesUpdatedAt(), deployTimestamp);
    });
  });
  ...
  describe("defaults", async function () {
    ...
    it("withdrawal fee is 50bps", async function () {
      await expectValue((await contracts.vault.feeStructure()).withdrawal, FEE_MULTIPLIER.mul(50));
    });

    it("management fee is 200bps", async function () {
      await expectValue((await contracts.vault.feeStructure()).management, FEE_MULTIPLIER.mul(200));
    });

    it("performance fee is 2000bps", async function () {
      await expectValue((await contracts.vault.feeStructure()).performance, FEE_MULTIPLIER.mul(2000));
    });

    it("vault share high water mark is 1e18", async function () {
      await expectValue(await contracts.vault.vaultShareHWM(), DEFAULT_HIGH_WATER_MARK);
    });

    it("assets checkpoint is 0", async function () {
      await expectValue(await contracts.vault.assetsCheckpoint(), 0);
    });
  });

  describe("pool token", async function () {
    it("generates token name from underlying", async function () {
      await expectValue(await contracts.vault.name(), "Popcorn Token Vault");
    });

    it("generates token symbol from underlying", async function () {
      await expectValue(await contracts.vault.symbol(), "pop-TOKEN");
    });
  ...
  describe("asset", async function () {
    it("stores the underlying token address", async function () {
      await expectValue(await contracts.vault.token(), await contracts.vault.asset());
    });
  });
  ...
  describe("assetsOf", async function () {
    ...
    it("returns the balance of tokens held by address", async function () {
      await expectValue(await contracts.vault.assetsOf(depositor.address), DEPOSIT_AMOUNT);
    });
    ...
    it("returns the balance of tokens when the vault value increases", async function () {
      ...
      await expectValue(await contracts.vault.assetsOf(depositor.address), DEPOSIT_AMOUNT.mul(2));
    });
  });
  ...
  describe("totalAssets", async function () {
    ...
    it("returns total assets after a deposit is made", async function () {
      ...
      await expectValue(await contracts.vault.totalAssets(), DEPOSIT_AMOUNT);
    });
    ...
    it("increases as vault value increases", async function () {
      ...
      await expectValue(await contracts.vault.totalAssets(), DEPOSIT_AMOUNT.mul(2));
    });
  });
  ...
  describe("previewDeposit", async function () {
    ...
    it("returns the amount of vault shares minted given the amount of assets deposited (no fee)", async function () {
      ...
      await expectValue(await contracts.vault.previewDeposit(DEPOSIT_AMOUNT), DEPOSIT_AMOUNT);
      await contracts.vault.connect(depositor)["deposit(uint256)"](DEPOSIT_AMOUNT);
      ...
      await expectValue(await contracts.vault.previewDeposit(DEPOSIT_AMOUNT), expectedShares);
    });
    ...
    it("returns the amount of vault shares minted given the amount of assets deposited (perfFee)", async function () {
      ...
      await expectValue(await contracts.vault.pricePerShare(), parseEther("1.8"));
    ...
    it("returns the amount of vault shares minted given the amount of assets deposited (depFee + perfFee)", async function () {
      ...
      await expectValue(totalSupply, DEPOSIT_AMOUNT);
      await expectValue(totalAssets, DEPOSIT_AMOUNT);
      await expectValue(userBal, expectedShares);
      ...
      await expectValue(totalAssets, DEPOSIT_AMOUNT.mul(2));
      await expectValue(userBal, expectedShares.mul(2));
      ...
      await expectValue(await contracts.vault.previewDeposit(DEPOSIT_AMOUNT), parseEther("550"));
      await contracts.vault.connect(depositor)["deposit(uint256)"](DEPOSIT_AMOUNT);
      ...
      await expectValue(await contracts.vault.balanceOf(depositor.address), expectedShares);
    ...
    it("prevents underflow on fee", async function () {
      ...
      await expectValue(await contracts.vault.previewDeposit(parseEther("1")), parseEther("1000000000000000000000"));
    });
  });
  ...
  describe("previewMint", async function () {
    ...
    it("returns the amount of assets required given the amount of shares wanted (no fee)", async function () {
      ...
      // Initial deposit mints 1 for 1
      await expectValue(await contracts.vault.previewMint(DEPOSIT_AMOUNT), DEPOSIT_AMOUNT);
  ...
  describe("previewWithdraw", async function () {
    ...
    it("returns the amount of shares needed to receive given amount of vaulted assets (no fee)", async function () {
      ...
      await expectValue(await contracts.vault.previewWithdraw(amount), amount);
      await contracts.vault.connect(depositor)["withdraw(uint256)"](amount);

      await expectValue(await contracts.vault.totalSupply(), 1);
      await expectValue(await contracts.vault.totalAssets(), 1);
      await expectValue(await contracts.vault.pricePerShare(), parseEther("1"));
    });
    ...
    it("returns the amount of shares needed to receive given amount of vaulted assets (withFee)", async function () {
      ...
      await expectValue(await contracts.vault.totalAssets(), parseEther("505"));
    ...
    it("returns the amount of shares needed to receive given amount of vaulted assets (perfFee)", async function () {
      ...
      await expectValue(await contracts.vault.previewWithdraw(DEPOSIT_AMOUNT), expectedSharesRequired);
      await contracts.vault.connect(depositor)["withdraw(uint256)"](DEPOSIT_AMOUNT);

      await expectValue(await contracts.vault.totalAssets(), DEPOSIT_AMOUNT);
      await expectValue(await contracts.vault.balanceOf(contracts.vault.address), expectedFeeInShares);
      await expectBigNumberCloseTo(await contracts.vault.pricePerShare(), parseEther("1.8"));
      await expectValue(await contracts.vault.balanceOf(depositor.address), DEPOSIT_AMOUNT.sub(expectedSharesRequired));
      await expectValue(
        await contracts.vault.totalSupply(),
    ...
    it("returns the amount of shares needed to receive given amount of vaulted assets (withdrawalFee + perfFee)", async function () {
      ...
      await expectValue(await contracts.vault.previewWithdraw(withdrawAmount), expectedSharesRequired);
      await contracts.vault.connect(depositor)["withdraw(uint256)"](withdrawAmount);

      // (DEPOSIT_AMOUNT * 2) - withdrawalAmount
      await expectValue(await contracts.vault.totalAssets(), parseEther("1505"));
      await expectBigNumberCloseTo(
  ...
  describe("previewMint", async function () {
    ...
    it("returns the amount of assets given an amount of shares (withFee)", async function () {
      ...
      await expectValue(await contracts.vault.totalAssets(), withdrawAmount.add(feeBalance));
      await expectValue(await contracts.vault.balanceOf(contracts.vault.address), feeBalance);
      await expectValue(await contracts.vault.pricePerShare(), parseEther("1"));
      await expectValue(await contracts.vault.balanceOf(depositor.address), withdrawAmount);
      await expectValue(await contracts.vault.totalSupply(), withdrawAmount.add(feeBalance));
    });
    ...
    it("returns the amount of assets given an amount of shares (perfFee)", async function () {
      ...
      await expectValue(await contracts.vault.totalAssets(), DEPOSIT_AMOUNT.mul(2).sub(expectedAssetsReturned));
      await expectValue(await contracts.vault.balanceOf(contracts.vault.address), expectedFeeInShares);
      await expectValue(await contracts.vault.pricePerShare(), parseEther("1.8"));
      await expectValue(await contracts.vault.balanceOf(depositor.address), withdrawAmount);
      await expectValue(await contracts.vault.totalSupply(), withdrawAmount.add(expectedFeeInShares));
    });
    ...
    it("returns the amount of assets given an amount of shares (withdrawalFee + perfFee)", async function () {
      ...
      await expectValue(await contracts.vault.totalAssets(), parseEther("1109"));
      await expectValue(await contracts.vault.balanceOf(contracts.vault.address), expectedFeeInShares);
      await expectValue(await contracts.vault.pricePerShare(), parseEther("1.8"));
      await expectValue(await contracts.vault.balanceOf(depositor.address), withdrawAmount);
      await expectValue(await contracts.vault.totalSupply(), withdrawAmount.add(expectedFeeInShares));
  ...
  describe("assetsPerShare", async () => {
    it("returns the same value as `convertToAssets`", async () => {
      await expectValue(await contracts.vault.assetsPerShare(), await contracts.vault.convertToAssets(parseEther("1")));
  ...
  describe("maxDeposit", async function () {
    it("returns max number of underlying assets that may be deposited", async function () {
      await expectValue(await contracts.vault.maxDeposit(depositor.address), ethers.constants.MaxUint256);
    });
    ...
    it("respects the underlying vault deposit limit", async function () {
      ...
      await expectValue(await contracts.vault.maxDeposit(depositor.address), depositLimit);
    });
    ...
    it("decreases as deposits increase", async function () {
      ...
      await expectValue(await contracts.vault.maxDeposit(depositor.address), depositLimit.sub(DEPOSIT_AMOUNT));
    });
    ...
    it("returns zero when deposit limit is reached", async function () {
      ...
      await expectValue(await contracts.vault.maxDeposit(depositor.address), 0);
    });
  });

  describe("maxMint", async function () {
    it("returns total number of share the caller can mint", async function () {
      await expectValue(await contracts.vault.maxMint(depositor.address), ethers.constants.MaxUint256);
    });

    it("respects the underlying vault deposit limit", async function () {
      const depositLimit = parseEther("100000");
      await contracts.yearnVault.setDepositLimit(depositLimit);
      await expectValue(await contracts.vault.maxMint(depositor.address), depositLimit);
    });
    ...
    it("decreases as deposits increase", async function () {
      ...
      await expectValue(await contracts.vault.maxMint(depositor.address), depositLimit.sub(DEPOSIT_AMOUNT));
    });
    ...
    it("returns zero when deposit limit is reached", async function () {
      ...
      await expectValue(await contracts.vault.maxMint(depositor.address), 0);
    });
  });
  ...
  describe("convertToShares/convertToAssets", async function () {
    ...
    it("convertToShares returns value of underlying asset in shares", async function () {
      await expectValue(await contracts.vault.convertToShares(parseEther("20")), parseEther("10"));
    });

    it("convertToAssets returns value of shares in amount of underlying asset", async function () {
      await expectValue(await contracts.vault.convertToAssets(parseEther("10")), parseEther("20"));
    });
  });
  ...
  describe("deposit", async function () {
    ...
    it("issues accrued fee shares", async function () {
      ...
      await expectValue(await contracts.vault.balanceOf(contracts.vault.address), 0);
      ...
      await expectValue(accruedManagementFee, parseEther("30"));
      await expectValue(accruedPerformanceFee, parseEther("200"));

      await contracts.vault.connect(depositor)["deposit(uint256)"](DEPOSIT_AMOUNT);

      const feeShareBalance = await contracts.vault.balanceOf(contracts.vault.address);
      await expectValue(feeShareBalance, parseEther("129.943502824858757062"));
      await expectBigNumberCloseTo(
    ...
    it("pool receives yearn shares in exchange for deposit", async function () {
      ...
      await expectValue(await contracts.yearnVault.balanceOf(contracts.vault.address), DEPOSIT_AMOUNT);
    });

    it("pool transfers vault shares to depositor", async function () {
      await contracts.vault.connect(depositor)["deposit(uint256)"](DEPOSIT_AMOUNT);

      await expectValue(await contracts.vault.connect(depositor).balanceOf(depositor.address), DEPOSIT_AMOUNT);
    });
    ...
    it("does not change the high water mark if share value is unchanged", async function () {
      await expectValue(await contracts.vault.vaultShareHWM(), parseEther("1"));
    ...
    it("updates the high water mark if share value has increased", async function () {
      ...
      await expectValue(await contracts.vault.vaultShareHWM(), parseEther("2"));
    });
    ...
    it("checkpoints assets on initial deposit", async function () {
      ...
      await expectValue(await contracts.vault.assetsCheckpoint(), DEPOSIT_AMOUNT);
    });
    ...
    it("caps fees to prevent underflow", async function () {
      ...
      await contracts.depositToken.connect(depositor).approve(contracts.vault.address, DEPOSIT_AMOUNT);
      await contracts.vault.connect(depositor)["deposit(uint256)"](DEPOSIT_AMOUNT);
      await expectValue(await contracts.vault.balanceOf(depositor.address), parseEther("1000000000000000000001000"));
    });
  });
    ...
    describe("fees", async function () {
      describe("management fees", async function () {
        ...
        it("takes a management fee when time passes", async function () {
          ...
          await timeTravel(365 * DAY);
          await expectValue(await contracts.vault.accruedManagementFee(), expectedFee);
        });
        ...
        it("doesn't take a management fee when no time passes", async function () {
          await expectValue(await contracts.vault.accruedManagementFee(), 0);
        });
      });
      ...
      describe("performance fees", async function () {
        ...
        it("takes a performance fee when value is more than HWM", async function () {
          ...
          await contracts.yearnVault.setPricePerFullShare(parseEther("2"));
          await expectValue(await contracts.vault.accruedPerformanceFee(), expectedFee);
        });
        ...
        it("doesn't take a performance fee when value is less than HWM", async function () {
          ...
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
        });
      });
      ...
      describe("calculate fees", async function () {
        it("calculates the correct management fee", async function () {
          ...
          await expectValue(await contracts.vault.accruedManagementFee(), parseEther(expectedManagementFee.toString()));
        });
        ...
        it("calculates the correct performance fee", async function () {
          ...
          await expectValue(
            await contracts.vault.accruedPerformanceFee(),
            parseEther(expectedPerformanceFee.toString())
          );
        });
      });
    });
  ...
  describe("takeManagementAndPerformanceFees", async function () {
    context("useLocalFees", () => {
      ...
      it("harvests performance fee", async function () {
        ...
        await expectValue(await contracts.vault.accruedPerformanceFee(), parseEther("1000"));
    ...
    context("use poolFeeController", () => {
      ...
      it("harvests performance fee", async function () {
        await contracts.depositToken.mint(contracts.yearnVault.address, parseEther("100000"));
        await expectValue(await contracts.vault.accruedPerformanceFee(), parseEther("20000"));
        await contracts.vault.takeManagementAndPerformanceFees();
        await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
        await expectBigNumberCloseTo(
          await contracts.vault.assetsOf(contracts.vault.address),
      ...
      it("harvests management fee", async function () {
        await timeTravel(365 * DAY);
        await expectValue(await contracts.vault.accruedManagementFee(), parseEther("20"));
        await contracts.vault.takeManagementAndPerformanceFees();
        await expectValue(await contracts.vault.accruedManagementFee(), 0);
        await expectBigNumberCloseTo(
          await contracts.vault.assetsOf(contracts.vault.address),
      ...
      it("updates vault share HWM", async function () {
        ...
        await contracts.vault.takeManagementAndPerformanceFees();
        await expectValue(await contracts.vault.vaultShareHWM(), parseEther("11"));
      });

      it("saves totalAssets checkpoint", async function () {
        await expectValue(await contracts.vault.assetsCheckpoint(), parseEther("1000"));
        await contracts.depositToken.mint(contracts.yearnVault.address, parseEther("10000"));
        await timeTravel(60);
        await expectValue(await contracts.vault.accruedManagementFee(), parseEther("0.000228310502283105"));
        await contracts.vault.takeManagementAndPerformanceFees();
        await expectValue(await contracts.vault.assetsCheckpoint(), parseEther("11000"));
      });
    });
    ...
    context("frequent calls", () => {
      ...
      it("daily harvests", async function () {
        for (let i = 0; i < 365; i++) {
          ...
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), parseEther("0.027397260273972602"));
          await contracts.vault.takeManagementAndPerformanceFees();
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), 0);
        }
      ...
      xit("twice-daily harvests", async function () {
        for (let i = 0; i < 2 * 365; i++) {
          ...
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), parseEther("0.013698630136986301"));
          await contracts.vault.takeManagementAndPerformanceFees();
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), 0);
        }
      ...
      it("one ten minute harvest", async function () {
        ...
        await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
        await expectValue(await contracts.vault.accruedManagementFee(), parseEther(".000190258751902587"));
        await contracts.vault.takeManagementAndPerformanceFees();
        await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
        await expectValue(await contracts.vault.accruedManagementFee(), 0);
        await expectBigNumberCloseTo(
      ...
      it("five two-minute harvests", async function () {
        for (let i = 0; i < 5; i++) {
          ...
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), parseEther("0.000038051750380517"));
          await contracts.vault.takeManagementAndPerformanceFees();
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), 0);
        }
      ...
      it("ten one-minute harvests", async function () {
        for (let i = 0; i < 10; i++) {
          ...
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), parseEther("0.000019025875190258"));
          await contracts.vault.takeManagementAndPerformanceFees();
          await expectValue(await contracts.vault.accruedPerformanceFee(), 0);
          await expectValue(await contracts.vault.accruedManagementFee(), 0);
        }
  ...
  describe("governance", async function () {
    ...
    describe("sending accrued fees to rewards manager", async function () {
      ...
      it("owner can withdraw accrued fees", async function () {
        ...
        await expectValue(await contracts.depositToken.balanceOf(rewardsManager.address), 0);
