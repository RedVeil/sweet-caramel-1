...
describe("XPopRedemption", () => {
  ...
  context("Constructor", async () => {
    it("stores address of the xPOP token", async () => {
      await expectValue(
        await contracts.xPopRedemption.xPOP(),
    ...
    it("stores address of the RewardsEscrow contract", async () => {
      await expectValue(
        await contracts.xPopRedemption.rewardsEscrow(),
  ...
  context("setApprovals", async () => {
    ...
    it("can only be called by owner", async () => {
      await expectRevert(
        contracts.xPopRedemption.connect(nonOwner).setApprovals(),
  ...
  describe("redeem", async () => {
    ...
    beforeEach(async () => {
      ...
      await contracts.rewardsEscrow
        .connect(owner)
        .addAuthorizedContract(contracts.xPopRedemption.address);
      await contracts.xPopRedemption.connect(owner).setApprovals();
    });
    ...
    context("successful redemption", async () => {
      ...
      it("transfers POP to escrow contract", async () => {
        await expectValue(
          await contracts.pop.balanceOf(contracts.rewardsEscrow.address),
          0
        );
    ...
    describe("redeem with signature", async () => {
      ...
      beforeEach(async () => {
        ...
        signature = await redeemer._signTypedData(
          data.domain,
          data.types,
          data.message
        );
      ...
      it("transfers in xPOP tokens", async () => {
        ...
        await contracts.xPopRedemption
          .connect(redeemer)
          .redeemWithSignature(XPOP_AMOUNT, defaultDeadline, v, r, s);
    ...
    describe("insufficient POP balance", async () => {
      it("fails fast on insufficient POP balance", async () => {
        await expectRevert(
          contracts.xPopRedemption.connect(redeemer).redeem(XPOP_AMOUNT),
