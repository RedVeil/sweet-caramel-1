...
import ButterBatchAdapter, {
  ComponentMap,
} from "../../lib/adapters/ButterBatchAdapter";
...
async function deployContracts(): Promise<Contracts> {
  ...
  const threePoolContract = (await ethers.getContractAt(
    CurveMetapoolAbi,
    threePool
  )) as CurveMetapool;
  ...
  const crvFraxMetapoolContract = (await ethers.getContractAt(
    CurveMetapoolAbi,
    crvFraxMetapool
  )) as CurveMetapool;

  const butter = (await ethers.getContractAt(
    SetTokenAbi.abi,
  ...
  const butterStaking = await (
    ...
    ).deploy(mockPop.address, butter.address, rewardsEscrow.address)
  ).deployed();
...
async function depositForHysiMint(
  ...
  await contracts.threeCrv
    .connect(account)
    .approve(contracts.butterBatch.address, threeCrvAmount);
...
async function distributeHysiToken(): Promise<void> {
  ...
  const [batchId] = await contracts.butterBatch.getAccountBatches(
    depositor.address
  );
  ...
  context("valuation oracle functions and slippage calculation", () => {
    ...
    describe("valueOfComponents", () => {
      it("returns value of components per unit", async () => {
        ...
        const value = await contracts.butterBatch.valueOfComponents(
          addresses,
          quantities
        );
        await expectBigNumberCloseTo(
          value,
          parseEther("1008.083393047704555181"),
          parseEther("0.00015")
        );
      });
    });
    ...
    describe("getMinAmount3CrvFromRedeem", () => {
      it("gets minimum amount of 3crv from provided butter balance", async () => {
        ...
        const minAmount =
          await contracts.butterBatch.getMinAmount3CrvFromRedeem(
            await contracts.butterBatch.valueOfComponents(
              addresses,
              quantities
            ),
            10
        ...
        await expectBigNumberCloseTo(
          minAmount,
          parseEther("987.396836874543984683"),
          parseEther("0.00015")
        );
      });
    });
  ...
  context("setters and getters", () => {
    describe("setCurvePoolTokenPairs", () => {
      it("sets curve pool token pairs", async () => {
        ...
        expect(
          await contracts.butterBatch.curvePoolTokenPairs(OTHER_TOKEN_ADDRESS)
        ).to.deep.eq([OTHER_TOKEN_ADDRESS, OTHER_TOKEN_ADDRESS]);
      });
    });
  });
  ...
  describe("mint", function () {
    context("depositing", function () {
      it("deposits 3crv in the current mintBatch", async function () {
        ...
        expect(
          await contracts.threeCrv.balanceOf(contracts.butterBatch.address)
        ).to.equal(parseEther("10"));
        const currentMintBatchId =
          await contracts.butterBatch.currentMintBatchId();
        const currentBatch = await contracts.butterBatch.batches(
          currentMintBatchId
        );
      ...
      it("adds the mintBatch to the users batches", async function () {
        await contracts.threeCrv
          .connect(depositor)
          .approve(contracts.butterBatch.address, parseEther("100"));
        await contracts.butterBatch
          .connect(depositor)
        ...
        const currentMintBatchId =
          await contracts.butterBatch.currentMintBatchId();
        expect(
          await contracts.butterBatch.accountBatches(depositor.address, 0)
      ...
      it("allows multiple deposits", async function () {
        ...
        expect(
          await contracts.butterBatch.accountBatches(depositor1.address, 0)
        ).to.equal(currentMintBatchId);
        expect(
          await contracts.butterBatch.accountBatches(depositor2.address, 0)
        ).to.equal(currentMintBatchId);
      });
    });
      ...
      context("sucess", function () {
        ...
        it("withdraws part of the deposit and continues to mint the rest", async function () {
          ...
          const suppliedTokenBalanceBefore = (
            await contracts.butterBatch.batches(batchId)
          ...
          expect(
            await contracts.butterBatch.connect(admin).batchMint()
          ).to.emit(contracts.butterBatch, "BatchMinted");
        });
      });
    });
    context("batch minting", function () {
      context("reverts", function () {
        it("reverts when minting too early", async function () {
          ...
          await contracts.butterBatch
            ...
            .depositForMint(parseEther("100"), depositor.address);
          await expect(
            contracts.butterBatch.connect(admin).batchMint()
        ...
        it("reverts when called by someone other the keeper", async function () {
          ...
          await expect(
            contracts.butterBatch.connect(depositor).batchMint()
      ...
      context("success", function () {
        ...
        it("advances to the next batch", async function () {
          ...
          const previousBatch = await contracts.butterBatch.batches(
            previousMintBatchId
          );
    ...
    context("claim batch", function () {
      ...
      it("claim batch successfully", async function () {
        const hysiBalanceBefore = await contracts.butter.balanceOf(
          depositor.address
        );
        ...
        const amountToReceive = await new ButterBatchAdapter(
          contracts.butterBatch
        ...
        const hysiBalanceAfter = await contracts.butter.balanceOf(
          depositor.address
        );
  ...
  describe("redeem", function () {
    ...
    context("depositing", function () {
      it("deposits setToken in the current redeemBatch", async function () {
        ...
        expect(
          await contracts.butter.balanceOf(contracts.butterBatch.address)
        ).to.equal(hysiBalance);
        const currentRedeemBatchId =
          await contracts.butterBatch.currentRedeemBatchId();
        const currentBatch = await contracts.butterBatch.batches(
      ...
      it("adds the redeemBatch to the users batches", async function () {
        ...
        const currentRedeemBatchId =
          await contracts.butterBatch.currentRedeemBatchId();
        expect(
          await contracts.butterBatch.accountBatches(depositor.address, 1)
      ...
      it("allows multiple deposits", async function () {
        ...
        expect(
          await contracts.butterBatch.accountBatches(depositor1.address, 1)
        ).to.equal(currentRedeemBatchId);
        expect(
          await contracts.butterBatch.accountBatches(depositor2.address, 1)
        ).to.equal(currentRedeemBatchId);
      });
    });
    context("withdraw from queue", function () {
      ...
      context("sucess", function () {
        it("withdraws the deposit", async function () {
          ...
          expect(await contracts.butter.balanceOf(depositor.address)).to.equal(
            hysiBalance
          );
        });
        it("withdraws part of the deposit and continues to redeem the rest", async function () {
          ...
          expect(
            await contracts.butterBatch.connect(admin).batchRedeem()
          ).to.emit(contracts.butterBatch, "BatchRedeemed");
        });
      });
    });
    ...
    context("batch redeeming", function () {
      context("reverts", function () {
        it("reverts when redeeming too early", async function () {
          ...
          await expect(
            contracts.butterBatch.connect(admin).batchRedeem()
          ).to.be.revertedWith("can not execute batch redeem yet");
        });
        it("reverts when called by someone other the keeper", async function () {
          await contracts.butter
            .connect(depositor)
            .approve(contracts.butterBatch.address, hysiBalance);
          await contracts.butterBatch
            .connect(depositor)
            .depositForRedeem(hysiBalance);
          await provider.send("evm_increaseTime", [2500]);
          ...
          await expect(
            contracts.butterBatch.connect(depositor).batchRedeem()
          ).to.be.revertedWith("you dont have the right role");
        });
        ...
        it("does not reverts with positive slippage", async function () {
          ...
          const batches = await new ButterBatchAdapter(
            contracts.butterBatch
          ).getBatches(depositor.address);

          await expect(
            batches[0].claimableTokenBalance.gt(minAmount)
          ).to.be.true;
        });
      });
      context("success", function () {
        it("batch emits a BatchRedeemedEvent", async function () {
          ...
          const result = await contracts.butterBatch
            .connect(admin)
            .batchRedeem();
  ...
  context("moveUnclaimedDepositsIntoCurrentBatch", function () {
    context("error", function () {
      ...
      it("skips a batch that is not from the correct batchType", async function () {
        await contracts.threeCrv
          .connect(depositor)
          .approve(contracts.butterBatch.address, parseEther("10000"));
        await contracts.butterBatch
          .connect(depositor)
      ...
      it("skips unclaimable batch", async function () {
        ...
        const batchIds = await contracts.butterBatch.getAccountBatches(
          depositor.address
        );
        const batches = await Promise.all(
          batchIds.map((id) => contracts.butterBatch.batches(id))
        );
    ...
    context("success", function () {
      it("moves minted butter into current redeemBatch", async function () {
        ...
        await provider.send("evm_mine", []);
        await contracts.butterBatch.connect(admin).batchMint();
        const mintedHYSI = await contracts.butter.balanceOf(
          contracts.butterBatch.address
        );
        expect(
          await contracts.butterBatch
        ...
        const currentRedeemBatchId =
          await contracts.butterBatch.currentRedeemBatchId();
        const redeemBatch = await contracts.butterBatch.batches(
          currentRedeemBatchId
        );
      ...
      it("moves 3crv redeemed from HYSI into current mintBatch", async function () {
        ...
        await contracts.butterBatch.connect(admin).batchRedeem();
        const redeemed3CRV = (await contracts.butterBatch.batches(batchId[1]))
      ...
      it("moves funds from up to 20 batches", async function () {
        ...
        await contracts.threeCrv
          .connect(depositor)
          .approve(contracts.butterBatch.address, parseEther("2000"));
        await bluebird.map(
          new Array(20).fill(0),
          async (i) => {
            await contracts.butterBatch
...
const depositForMint = async (depositor, amount: BigNumber) => {
  await contracts.threeCrv
    .connect(depositor)
    .approve(contracts.butterBatch.address, amount);
  return await contracts.butterBatch
    .connect(depositor)
    .depositForMint(amount, depositor.address);
};
