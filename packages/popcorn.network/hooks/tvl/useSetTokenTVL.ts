...
import { ChainId, PRC_PROVIDERS, RPC_URLS } from "web3/connectors";
...
export async function getSetTokenTVL(key, setTokenAddress: string, batchAddress: string, rpcProvider): Promise<BigNumber> {
  ...
  const setToken = new ethers.Contract(setTokenAddress, ["function totalSupply() external view returns (uint256)"], rpcProvider);
...
export default function useSetTokenTVL(setTokenAddress: string, batchAddress: string): SWRResponse<BigNumber, Error> {
  return useSWR([`getSetTokenTVL-${setTokenAddress}`, setTokenAddress, batchAddress, PRC_PROVIDERS[ChainId.Ethereum]], getSetTokenTVL, {
